...
| (4) 静态量化 | 可选（文档原文"做不做都行，能讲清楚就行"） | 原理已可完整复述（校准/写入时量化/scale 挂层/chunk_prefill 反量化/debug 二分法），代码未写 |
...
\* aime25：lm-eval 该任务为裸补全格式（不套 chat template）且难度远超 9B，基线即 0/30，基准失去分辨力；叠加 int8 场次在 32k 上下文下单场需 3.5h（见 4.3），经权衡砍掉，以 gsm8k 全量补位数学轴。文档要求的 aime 轴存在此缺口，理由可作为实验观察陈述；如需补齐可带 chat template 重跑。
...
- 速度（8 题 gsm8k 生成段）：原生 8s ≈ bf16 版 9s ≈ 提速后 int8 版 9s（首版整池量化 59s）。
