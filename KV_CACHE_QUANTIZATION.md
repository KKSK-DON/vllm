...
方法:~7.5k token 公共前缀 + 两个不同问题**串行**提交（第二问应命中第一问留下的前缀块）;`enable_prefix_caching` 开/关 × 三模式共 6 场（混合架构默认不开,须显式传 True,见 F 前文 opt-in 考证）;脚本 `prefix_test.sh`/`prefix_cache_probe.py`,关键输出带 `TIME_`/`TEXT_` 标记前缀直接 grep 提取。

| 模式 | 第二问耗时(关缓存) | 第二问耗时(开缓存) | 输出文本 |
| --- | --- | --- | --- |
| 原生 | 0.72s | **0.49s（−32%）** | 开关逐字相同 |
| `int8_static_per_channel` | 0.93s | **0.54s（−42%）** | 开关逐字相同 |
| `int8_phys_per_channel`（int8 池） | 0.95s | **0.59s（−38%）** | 开关逐字相同 |

三条结论:① 混合架构 opt-in 开启后 prefix caching 可正常工作,**含 int8 物理池**;② 物理模式下被共享的块就是 int8 码,第二请求直接复用并反量化,输出零变化——「全局时不变 scale ⇒ 缓存块可共享」的构造性论证获得实测支持;③ 三模式省时幅度一致（~0.2-0.4s,量级≈被跳过的前缀预填充成本）。诚实边界:命中证据是间接的（耗时差,未读命中率计数器）;文本对拍窗口 16 token;只测了 per_channel（per_head 走完全相同的查表机制）。
