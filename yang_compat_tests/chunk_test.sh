...
# 逐级对拍 native / yang bf16 / int8_static / int8_phys 的生成文本。
...
YANG_ATTN_MODE=int8_static_per_channel python $EV/chunk_prefill_probe.py > $EV/CH_static_pc.out 2> $EV/CH_static_pc.log || { touch $EV/.chunk.fail; exit 1; }
echo "[chunk] phys_pc START $(date +%H:%M:%S)"
YANG_ATTN_MODE=int8_phys_per_channel KVD=int8 python $EV/chunk_prefill_probe.py > $EV/CH_phys_pc.out 2> $EV/CH_phys_pc.log || { touch $EV/.chunk.fail; exit 1; }
# vLLM 日志走 stdout,与生成文本同流;diff 前先抠出"引擎谢幕行之后"的纯文本
for f in native yang_bf16 static_pc phys_pc; do
...
echo "=== static_pc vs phys_pc ==="; diff $EV/T_static_pc.txt $EV/T_phys_pc.txt && echo SAME_3
touch $EV/.chunk.done
